...
func NewTCPClientHandler(address string, rack int, slot int, customConnectionType ...int) *TCPClientHandler {
	h := &TCPClientHandler{}
	h.Address = address
	h.Timeout = tcpTimeout
	h.IdleTimeout = tcpIdleTimeout
	//custom connection type
	if len(customConnectionType) > 0 {
		h.ConnectionType = customConnectionType[0] // Connect to the PLC as a OP or Other
	} else {
